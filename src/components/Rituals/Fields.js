...
import { updateRitual, newRitual, getPlayables } from 'services/ritual';
  ...
  <>
    <li key={id} data-id={id} data-type={type} className='drag-drop-track my-2'>
      <span className='mr-3'>
        ...
        {type === 'Ritual' && <PlaySquareOutlined />}
      </span>
...
const RitualFields = ({ id, initialValues, action, history }) => {
  ...
  const handleSearch = (value) => {
    ...
    if (requestReference === 'Ritual') {
      apiTarget = 'rituals';
      setPrevRitualKeyword(value);
      setPrevTrackKeyword(undefined);
    } else if (requestReference === 'Track') {
      apiTarget = 'tracks';
      setPrevTrackKeyword(value);
      setPrevRitualKeyword(undefined);
    }
    const url = urlWithParams(`${apiEndpoint}/${apiTarget}`, params);

    if (prevTrackKeyword !== undefined || prevRitualKeyword !== undefined) {
  ...
  const handleRemoveItemFromRitual = (id) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className='row'>
          <div className='col-lg-12'>
            <Form.Item name='tag' label='Select tag for ritual'>
              <CustomSelect
                value={tagValue}
                loading={fetchingTag}
          ...
          <div className='col-lg-6'>
            <Item
              ...
              rules={[
                {
                  required: false,
                  message: 'Track is required to create ritual',
                },
              ]}
            >
              <div className='row'>
              ...
              <div className='row my-5'>
                <div className='col-lg-12'>
                  <h4 className='mx-1 d-flex justify-content-center'>
                    {dndItems.length ? 'Order Tracks and Ritual' : ''}
                  </h4>
                  <div className='box-h250 px-5'>
                    {requestingPlayables ? (
                      ...
                      <SortableList
                        ...
                        pressDelay={200}
                        onDelete={handleRemoveItemFromRitual}
                      />
                    )}
...
export default withRouter(RitualFields);
