...
const baseUrlCampaign = `${apiEndpoint}/${campaignUrl}`;
...
export const getAllCampaign = (obj) => {
  let { params, callback, prevCampaignKeyword } = obj;
  let url = urlWithParams(baseUrlCampaign, params);

  return (dispatch) => {
    if (prevCampaignKeyword !== undefined) {
      getApiCall({ url, dispatch, abort: true })
        .then((resp) => {
          if (resp !== 0 && resp !== undefined) {
            dispatch(setCampaignData(resp.data));
            dispatch(
              setCampaignMeta({
        ...
        .finally(() => callback());
    } else if (prevCampaignKeyword === undefined) {
...
export const getCampaign = (obj) => {
  ...
  return (dispatch) => {
    getApiCall({ url, dispatch })
      .then((resp) => {
        if (resp !== 0) {
          dispatch(setCampaign(resp.data));
        } else {
